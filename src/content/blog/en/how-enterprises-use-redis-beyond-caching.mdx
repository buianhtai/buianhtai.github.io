---
title: "How Enterprises Use Redis Beyond Caching"
description: "Cache, ID generator, distributed lock — that's where most teams stop with Redis. The companies running it hardest use it as an in-memory data-structures server: rate limiters, leaderboards, message streams, probabilistic counters. Here are the patterns, when to reach for each, and where they bite."
heroImage: "/og/redis-og.png"
pubDate: 2026-08-24
tags: ["redis", "system-design", "architecture", "scalability", "data-structures"]
lang: "en"
...
<Callout color="teal">
The mental model shift: stop asking "what can I cache?" and start asking "which of Redis's eight data structures matches my access pattern?" Sorted Set, Stream, HyperLogLog, Bitmap, Hash, List, Pub/Sub, Geospatial — each one is a production use case waiting to be recognized.
</Callout>
...
<DataTable
  caption="The five rate-limiting algorithms"
  accent="teal"
  headers={["Algorithm", "Memory/client", "Accuracy", "Burst behavior", "Best for"]}
  rows={[
    { "Algorithm": "Fixed window", "Memory/client": "1 key", "Accuracy": "Approximate", "Burst behavior": "2× burst at boundaries", "Best for": "Simple API limits, login throttling" },
    { "Algorithm": "Sliding window log", "Memory/client": "O(n) entries", "Accuracy": "Exact", "Burst behavior": "None", "Best for": "High-value APIs, audit trails" },
    { "Algorithm": "Sliding window counter", "Memory/client": "2 keys", "Accuracy": "Near-exact", "Burst behavior": "Smoothed boundaries", "Best for": "General-purpose APIs" },
    { "Algorithm": "Token bucket", "Memory/client": "1 hash", "Accuracy": "Exact", "Burst behavior": "Controlled bursts", "Best for": "Bursty traffic with average-rate caps" },
    { "Algorithm": "Leaky bucket", "Memory/client": "1 hash", "Accuracy": "Exact", "Burst behavior": "Strict drain, no bursts", "Best for": "Policing (reject) or shaping (delay)" },
  ]}
/>
...
<Flowchart
  ...
  nodes={[
    { id: "start", label: "Pick your limiter",        shape: "oval",    color: "teal",  icon: "simple-icons:redis", sub: "shared across all instances" },
    { id: "exact", label: "Exact counts required?",   shape: "diamond",                 sub: "billing, audit, SLA enforcement" },
    { id: "vol",   label: "High request volume?",     shape: "diamond",                 icon: "lucide:gauge", sub: "memory scales with traffic" },
    { id: "log",   label: "Sliding window log",       shape: "rect",    color: "violet",sub: "sorted set of timestamps, O(n)" },
    { id: "ctr",   label: "Sliding window counter",   shape: "rect",    color: "teal",  sub: "weighted blend of 2 windows, O(1)" },
    { id: "burst", label: "Bursts acceptable?",       shape: "diamond",                 sub: "spikes above average rate" },
    { id: "tok",   label: "Token bucket",             shape: "rect",    color: "green", icon: "lucide:zap", sub: "capacity + refill rate" },
    { id: "fix",   label: "Fixed window",             shape: "rect",    color: "amber", sub: "cheapest; 2× at boundaries" },
  ]}
  edges={[
    { from: "start", to: "exact" },
    { from: "exact", to: "vol",   label: "yes" },
    { from: "exact", to: "burst", label: "no" },
    { from: "vol",   to: "log",   label: "moderate" },
    { from: "vol",   to: "ctr",   label: "high" },
    { from: "burst", to: "tok",   label: "yes" },
    { from: "burst", to: "fix",   label: "no" },
  ]}
/>
...
<DataTable
  caption="Three messaging primitives, three contracts"
  accent="violet"
  headers={["Feature", "Pub/Sub", "Lists", "Streams"]}
  rows={[
    { "Feature": "Persistence", "Pub/Sub": "No — fire-and-forget", "Lists": "Yes", "Streams": "Yes — append-only log" },
    { "Feature": "Consumer groups", "Pub/Sub": "No", "Lists": "No", "Streams": "Yes" },
    { "Feature": "Replay history", "Pub/Sub": "No", "Lists": "No", "Streams": "Yes — read from any point" },
    { "Feature": "Acknowledgment", "Pub/Sub": "No", "Lists": "Manual", "Streams": "Built-in (XACK)" },
    { "Feature": "Delivery guarantee", "Pub/Sub": "At-most-once", "Lists": "At-most-once", "Streams": "At-least-once" },
    { "Feature": "Read effect", "Pub/Sub": "Broadcast", "Lists": "Destructive pop", "Streams": "Cursor moves, data stays" },
  ]}
/>
...
<StateMachine
  title="stream message lifecycle under a consumer group"
  states={[
    { id: "logged",  label: "In the log",   icon: "lucide:database", sub: "XADD — immutable entry",            row: 1, col: 1 },
    { id: "pending", label: "Pending (PEL)",icon: "lucide:clock-3", sub: "delivered, awaiting XACK",          row: 1, col: 2 },
    { id: "acked",   label: "Acked",        icon: "lucide:check-circle", sub: "processed; group cursor advances",  row: 1, col: 3 },
    { id: "claimed", label: "Claimed",      sub: "XAUTOCLAIM — new owner",            row: 2, col: 2 },
    { id: "dlq",     label: "Dead-lettered",sub: "times-delivered over threshold",    row: 2, col: 3 },
  ]}
  transitions={[
    { from: "logged",  to: "pending", label: "XREADGROUP >" },
    { from: "pending", to: "acked",   label: "XACK after success" },
    { from: "pending", to: "claimed", label: "owner idle past threshold" },
    { from: "claimed", to: "dlq",     label: "delivery count exceeded" },
  ]}
/>
...
<SequenceDiagram
  title="consumer crash and work stealing"
  actors={[
    { label: "Producer", color: "teal", icon: "lucide:upload" },
    { label: "Redis Stream", color: "blue", icon: "simple-icons:redis" },
    { label: "Worker A", color: "violet", icon: "lucide:cpu" },
    { label: "Worker B", color: "green", icon: "lucide:cpu" },
  ]}
  messages={[
    { from: "Producer", to: "Redis Stream", label: "XADD order-event", type: "request" },
    { from: "Worker A", to: "Redis Stream", label: "XREADGROUP — receives msg 1526...", type: "request" },
    { note: "Worker A crashes before processing — msg sits in the Pending Entries List." },
    { from: "Worker B", to: "Redis Stream", label: "XAUTOCLAIM (idle > 60s)", type: "request" },
    { from: "Redis Stream", to: "Worker B", label: "hands over msg 1526... + delivery count", type: "response" },
    { from: "Worker B", to: "Redis Stream", label: "process, then XACK", type: "request" },
    { note: "Delivery count over N attempts = poison pill: route to a dead-letter queue in application code." },
  ]}
/>
...
<MetricBar
  title="HyperLogLog keeps cardinality memory bounded"
  metrics={[
    { label: "1K uniques", value: 12, displayValue: "~12KB", color: "teal" },
    { label: "1B uniques", value: 12, displayValue: "~12KB", color: "green" },
    { label: "Set at 1B", value: 1000000, displayValue: "≈1M× larger", color: "amber" },
  ]}
/>
...
<Flowchart
  title="what kind of lock do you actually need?"
  nodes={[
    { id: "q",     label: "If two clients held the lock once, what breaks?", shape: "diamond",                 sub: "ask honestly" },
    { id: "eff",   label: "Duplicate work, absorbed downstream",              shape: "rect", color: "teal",  sub: "idempotent consumers, re-run costs money only" },
    { id: "single",label: "Single-instance lock",                             shape: "rect", color: "green", sub: "SET NX PX + Lua compare-and-delete" },
    { id: "corr",  label: "Corruption or money moves twice",                  shape: "rect", color: "rose",  sub: "correctness depends on the lock" },
    { id: "cons",  label: "Consensus system + fencing tokens",               shape: "rect", color: "violet",sub: "ZooKeeper/etcd; storage rejects stale tokens" },
  ]}
  edges={[
    { from: "q",     to: "eff",   label: "annoying, not fatal" },
    { from: "eff",   to: "single" },
    { from: "q",     to: "corr",  label: "catastrophic" },
    { from: "corr",  to: "cons" },
  ]}
/>
...
<Callout color="amber">
The litmus test for every pattern above: Redis earns its place when the workload needs shared, fast, small-lived state — counters that expire, rankings that reorder, messages that acknowledge. The moment state must survive forever or stay perfectly consistent under partition, hand it to a durable database and let Redis do what it's brilliant at.
</Callout>
